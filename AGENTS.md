...
- Otakon catalog maps: Dealers + Artist Alley (`public/samples/otakon-2026-*`). Unofficial sheet masterlists live beside them (`*-masterlist.json`); refresh with `python3 scripts/sync-otakon-masterlists.py` (needs `openpyxl`). Bump `SAMPLE_REVISION` in `server/catalogStore.ts` when samples/masterlists change so Render Postgres reseeds on next `/api/sync/catalog`. The catalog bundle includes `masterlists` plus per-booth `catalogInfo` for offline Booth info.
