...
- Otakon catalog maps: Dealers + Artist Alley (`public/samples/otakon-2026-*`). Bump `SAMPLE_REVISION` in `server/catalogStore.ts` when samples change so Render Postgres reseeds on next `/api/sync/catalog`.
