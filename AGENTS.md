...
- PWA service worker is disabled in Vite dev (`devOptions.enabled: false`), so hot reload is fine — no stale SW cache gotcha while on `npm run dev`. Update toast / Refresh only appear under `npm run preview` or production (`registerType: 'prompt'`). Changelog + version live in the map hamburger footer; source text is `src/lib/changelog.ts` (`APP_VERSION`).
